...
byte-reproducible, so a release **preserves and validates** the committed payload;
it never replaces it. The source bundle generated by the recipe must be staged
and published beside the installer. Regenerating a missing source bundle
currently also compiles a scratch `ffmpeg.exe` under `artifacts\ffmpeg`, which is
discarded.
...
if (-not (Test-Path -LiteralPath $ffmpegManifest -PathType Leaf)) {
    # Fresh machine only: regenerate the source bundle (needs MSYS2; this also
    # compiles a scratch ffmpeg.exe under artifacts). Without -InstallPayload the
    # committed ffmpeg.exe and its provenance stay untouched.
...
Preserve and validate the committed payload; a release never replaces it (see
