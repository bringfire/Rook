...
def main() -> None:
    ...
    write_cancel_evidence(
        "p3",
        ctx,
        "Replicate exposes `urls.cancel` in the prediction submit response (e.g., "
        "`https://api.replicate.com/v1/predictions/{id}/cancel`). "
        "Live cancellation was not exercised against the running prediction. "
        ...
        "evidence that Replicate supports cancellation; the HTTP method (POST per Replicate docs) "
        "and billing/idempotency semantics can be characterized at Phase 1 implementation time.",
    )
