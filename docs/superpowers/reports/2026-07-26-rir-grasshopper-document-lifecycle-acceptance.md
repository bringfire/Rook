...
The managed Grasshopper production paths indexed in the [normative design](../specs/2026-07-26-rir-grasshopper-document-lifecycle-design.md#appendix-a-rook2-reimplementation-index) were verified byte-for-byte equal between the original accepted tree (whose report commit was `23122e79`) and replacement head `63ce9c2c`. The indexed Python consumer path intentionally differs and is validated by the replacement-head MCP smoke. The replacement therefore relies on the original full host matrix only for the byte-identical managed paths; the Python boundary has its own live evidence below.
