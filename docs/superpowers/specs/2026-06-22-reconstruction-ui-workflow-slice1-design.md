...
## 8. Testing strategy
- **C# (xUnit):** focused test for the reconstruction bridge timeout wrapper — assert `submit_job/job_status/cancel_job` **route through** the shared `DispatchWithTimeoutAsync`/`AsyncOpTimeout` path. The pure timeout/rewrite behavior of `DispatchWithTimeoutAsync` already has robust coverage; do **not** duplicate it. The new test only needs to prove the three reconstruction async ops invoke that helper (e.g. an overrunning dispatch surfaces the helper's structured timeout failure), not re-verify all timeout semantics. `Rook.Tests` already covers `ReconstructionOpHandler`.
- **JS:** the Vision UI has no JS unit harness; UI behavior is verified manually (consistent with how Generate/Video views are validated). Provide a **manual verification checklist** in the plan: model list loads & forces explicit pick; textured/geometry-only mutual exclusion; Send-to-3D preselects & navigates; submit→poll→result; **warnings render** for a `result_missing_texture` case using a known bare-package fixture/test path, or by selecting a completed job whose `job_result.warnings[]` already includes `result_missing_texture` — **geometry-only must produce no texture warning** (it sets `TextureExpected=false`, which intentionally suppresses `result_missing_texture` per the D2/D3 contract; do not use geometry-only as the repro). If a live bare-package result is impractical to produce, treat the warnings-render check as a **test/data-injection** item (feed a synthetic `job_result` with the warning) rather than a live-smoke claim; job history refresh + click-to-load; blob thumbnail resolves.
