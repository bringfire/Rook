...
3. **no importable role → null.** Package contains no role named by `preferred_asset` or `fallback_order`; assert `resolved_import_role == null` and that reading the summary does not throw.
