...
| **registry-claim failed** (`insert_launching`/`bind` threw — DB lock/schema/disk) | attempts cleanup, then **delete the row only on confirmed death** (§8 L1): **reaped** → `{cleanupStatus:"forced_kill", retryable:true}` (any row dropped, no `session`); **reap failed** (process still alive) → the claim is RETAINED, or rescue-inserted if the original insert never landed → `{session:"rhino-<pid>", lifecycleStatus:"launching", port:null, cleanupStatus:"force_kill_failed", retryable:true}`; **reap failed AND no row could be made** (registry fully unavailable) → `{unclaimableResidual:true, cleanupStatus:"force_kill_failed", retryable:false}`, a reported live-process residual. A catchable registry failure never *silently* orphans a live process. |
