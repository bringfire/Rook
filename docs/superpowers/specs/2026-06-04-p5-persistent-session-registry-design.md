...
| **registry-claim failed** (`insert_launching`/`bind` threw — DB lock/schema/disk) | attempts cleanup, then **delete the row only on confirmed death** (§8 L1), with the **code distinguishing failure from recovery**: **reaped** → `{code:"workbench_registry_claim_failed", cleanupStatus:"forced_kill", retryable:true}` (any row dropped, no `session`); **reap failed** (process still alive) → the claim is RETAINED, or rescue-inserted if the original insert never landed → `{code:"workbench_launch_retained_after_registry_error", session:"rhino-<pid>", lifecycleStatus:"launching", port:null, cleanupStatus:"force_kill_failed", retryable:true}` — **RECOVERED**: the session is listable/closable, *not* a failed claim; **reap failed AND no row could be made** (registry fully unavailable) → `{code:"workbench_registry_claim_failed", unclaimableResidual:true, cleanupStatus:"force_kill_failed", retryable:false}`. A catchable registry failure never *silently* orphans a live process. |
| **panel-scope refused** (defense-in-depth, *before* `Popen`) | `{success:false, data:{ code:"workbench_requires_external_scope", message, retryable:false }}` — a panel-scoped runtime launches nothing; no Rhino is ever spawned (§10) |

The load-bearing fixes: (1) a `force_kill_failed` launch (bind-timeout *or* registry-claim) **returns the `session` handle + `lifecycleStatus:"launching"`** and `list` includes `launching` rows — otherwise the coordinator owns a durable live row it can neither see nor close; a registry-claim failure **deletes the row only on confirmed death**, else retains/rescues a durable claim (never silently orphans). (2) launch checks scope **before `Popen`**, so a panel-scoped runtime acquires nothing.
...
- **Lifecycle totality as EXECUTABLE data (Codex finding 3):** two complementary Python-data tests, NOT scraped from this markdown. **(a) A canonical decision table** pins the §8.3 rows to exact expected `Decision`s. **(b) A full-product invariant sweep** enumerates `(status × port_is_null × event × rhino_alive × owner_alive × scope × rebind_available)` and asserts, for *every* tuple, that `decide` is total, that **no `Decision` leaves a row at rest in `closing`** (post-state ∈ {`bound`, `launching`, deleted}), and that reclaim is gated (external + dead-owner + live-rhino). The table proves the exact edges; the sweep proves the invariants hold across the whole space. Together they are the regression guard against the whole P1 class.
