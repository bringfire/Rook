...
| **registry-claim failed** (`insert_launching`/`bind` threw — DB lock/schema/disk) | `{success:false, data:{ code:"workbench_registry_claim_failed", message, processId, cleanupStatus:"forced_kill"\|"force_kill_failed", retryable }}` — the live Rhino is **reaped, not orphaned**: a catchable registry failure must never leave a live process with no row (§8 L1) |
...
The load-bearing fixes: (1) a `force_kill_failed` launch **returns the `session` handle + `lifecycleStatus:"launching"`** and `list` includes `launching` rows — otherwise the coordinator owns a durable live row it can neither see nor close; and a registry-claim failure **reaps** rather than orphans. (2) launch checks scope **before `Popen`**, so a panel-scoped runtime acquires nothing.
