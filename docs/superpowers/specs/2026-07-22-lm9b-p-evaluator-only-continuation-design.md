...
The sealed verifier reconstructs both the instrument and evaluator result rather than trusting their authored identities. It binds archived source bytes to the locked production pins, substitutes the exact code-owned corrected rubric through the shared frozen-input constructor, recomputes the mechanical gate and evaluator renderer, and rebuilds the provider request from the fixed system prompt and shared builder. It requires the complete protocol identity, allowed delta, gate, rendered request, and provider request to match those constructive results byte-for-byte. It then reconstructs the evaluator result from the normalized assistant message and exact archived tool arguments through the same pure parser used by live evaluation, reruns readiness from archived inputs, validates invocation and dispatch equations, and derives classification from the reconstructed evaluator result and exact recipe bytes. A checksummed authored claim is never accepted as its own authority.

During execution, the private staging/finalization verifier receives the already verified in-memory corrected-rubric bytes from the frozen execution snapshot; it does not reread the external fixture after reservation. The public verifier independently loads the code-owned rubric associated with the implementation used to verify an archive.
