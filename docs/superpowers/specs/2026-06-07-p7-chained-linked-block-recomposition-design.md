...
The session-ambiguity guard is the **first** operation and is read-only — it must run before any mutation, or with multiple live Rhinos the harness could mutate whichever instance ambient routing picks before the guard fires. It replaces the `fresh_document` fixture (which would `new` a document during setup, *before* the guard). The standing throwaway-Rhino confirm-in-chat rule still applies (the harness calls `rhino_document_ops(action="new"/"open")`).

0. **Session guard (first, read-only):** read `rhino_sessions`; filter `liveness.state == "live"`. **Zero live → `pytest.skip`** (held live test; nothing running). **More than one live → fail** with an explicit blocked-for-safety message — `"multiple live Rhino sessions; close the extras, or run under an owned Workbench in a later slice. Harness safety guard, not a product failure."` Bind the single live session `S`.
1. Create + save `s1`, `s2`, `intermediate`, `master` on `S`: for each, `rhino_document_ops(action="new")` → `rhino_create` a distinguishing box → `rhino_document_ops(action="save", path=…)`, **asserting each `new` / `create` / `save` did not fail** (a failed create must not leave an empty saved source that silently passes the linked-block path). P6-register each via `artifacts.register_artifact` and capture its `artifactId`.
...
5. Idempotent re-run: for each plan step in order, while its target is active snapshot its Rook deterministic-name set (`rook_p7lb_*` from `/blocks`), then re-open + re-execute and assert outcome `refreshed_existing` + `saved` + the snapshot **unchanged before vs. after** (§5 no-dup, nesting-agnostic).
6. Record observations (§6). For the persisted-reload characterization, **reopen `master` via an explicit eviction step**: open a fresh blank document (`rhino_document_ops(action="new")`) — or another temp doc — to evict `master` from the single active slot, then `rhino_document_ops(action="open", path=master)` again. A bare "reopen" while `master` is already active would not exercise the on-disk reload and would prove nothing. Clean up temp files in `finally`.
...
- The exact `sourcePath` form Rhino records per linked def (absolute vs. relative; how the intermediate's path appears *inside* master), **after save** and **after an evict-then-reopen of master** (open a blank/other doc first, then reopen `master` — see §4.2 step 6).
