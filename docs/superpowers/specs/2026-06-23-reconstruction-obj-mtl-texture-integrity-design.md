...
### Part 1 — Classify texture roles by filename, consistently in *both* texture-keyed sites
There are **two** places that key fal texture URLs to roles, and they must agree:
- the result mapper [`FalReconstructionResultMapper.MapArtifacts`](../../../src/Rook/Services/Reconstruction/Fal/FalReconstructionResultMapper.cs) (assigns the package's file roles), and
- the import-prep provider-filename resolver [`ReconstructionOpHandler.ProviderFileNamesByRole`](../../../src/Rook/Handlers/ReconstructionOpHandler.cs) (maps role → original provider filename so staged blobs are renamed to the names the MTL references).

Both currently classify only `texture_urls.*` via `ClassifyTextureRole` and key the **top-level `texture`** (and `model_urls.texture`) under the **generic** `texture` role (mapper line 65-66; resolver line 774). Fix **both** to run **top-level `texture`, `model_urls.texture`, and `texture_urls.*`** through the *same* `ClassifyTextureRole(fileName, url)`.

**Invariant (the load-bearing requirement):** for any given provider texture file, the role the mapper stores it under and the role the resolver keys its provider filename under **must be identical** — otherwise `FileNameForRole(role)` misses and the staged blob is renamed to a role-derived name (`texture_metallic.png`) instead of the MTL-referenced provider name (`texture_pbr_v128_metallic.png`), breaking even the maps that *are* present. For this package: the metallic lands as `texture_metallic` in **both** sites, so `FileNameForRole("texture_metallic")` resolves to `texture_pbr_v128_metallic.png`, staging preserves it, and the MTL's `map_Pm` reference resolves (while `map_Kd` is correctly absent → Part 3). This also recovers any future payload that ships detailed maps in `texture`/`model_urls`.
...
- `src/Rook/Services/Reconstruction/Fal/FalReconstructionResultMapper.cs` — Part 1 (mapper side): classify top-level `texture` + `model_urls.texture` via the existing `ClassifyTextureRole` instead of the generic role.
- `src/Rook/Handlers/ReconstructionOpHandler.cs` — Part 1 (resolver side): `ProviderFileNamesByRole` must classify top-level `texture` (line 774) + `model_urls.texture` via the **same** `ClassifyTextureRole`, so its role keys match the mapper's (texture_urls.* already does). This keeps `FileNameForRole` resolving the real provider name for staging.
...
- `src/Rook/Services/Reconstruction/ReconstructionJobManager.cs` — Part 3/4: in the `Result()` path, an MTL-integrity step that reads the package MTL blob, recovers delivered provider filenames from `provider_result_json` (reuse the now-consistent `ProviderFileNamesByRole` authority — extract/share it rather than duplicating), compares `map_*` refs against the delivered filename set, and **appends** a `material_maps_missing` warning to the envelope's `warnings[]`. A new `material_maps_missing` warning-code constant.
...
- **Resolver/mapper agreement (the consistency invariant):** for the literal payload, `ProviderFileNamesByRole` keys the top-level metallic under `texture_metallic` (matching the mapper), so `FileNameForRole("texture_metallic")` → `texture_pbr_v128_metallic.png` (not a role-derived name). Assert the resolver and mapper assign the same role to the same provider file.
