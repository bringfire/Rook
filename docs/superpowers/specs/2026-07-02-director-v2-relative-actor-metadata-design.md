...
- Director take manifests, full-model capture indexes, depth-test metadata, and
  other broader prototype archive files.
...
- starting from no `.rook`, the production actor metadata writer entry point
  ...
  the active V2 `.3dm` and a minimal known actor-authoring input: the roof-uplift
  actor set target, the same-orientation mullion subset, and the accepted band
  grouping used by the prototype;
...
- migrate Director take archives or full-model capture indexes;
