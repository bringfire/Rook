...
| Annotation | `rhino_annotation_text`, `rhino_annotation_dot`, `rhino_annotation_leader`, `rhino_annotation_dim_angle`, `rhino_annotation_dim_radius`, `rhino_annotation_dim_diameter`, `rhino_annotation_dim_aligned`, `rhino_annotation_dim_linear` | typed MCP tools -> native annotation routes | annotation object ids / measured value for dimensions | Use instead of raw text/dimension command prompts. |
...
| Blocks | `rhino_blocks`, `rhino_block_create`, `rhino_block_insert`, `rhino_block_info`, `rhino_block_add_objects`, `rhino_block_remove_objects`, `rhino_block_replace_object_geometry`, `rhino_block_transform_object`, `rhino_block_set_layers`, `rhino_block_set_materials`, `rhino_block_set_object_colors`, `rhino_block_set_object_names`, `rhino_block_set_object_user_strings`, `rhino_block_find_instances`, `rhino_block_objects_detailed` | typed MCP tools -> native/managed routes | block name, instance ids, mutation summary | Some block-definition mutation routes are managed by design; preserve current ownership. |
