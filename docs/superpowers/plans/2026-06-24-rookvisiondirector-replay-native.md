...
- **Error remapping (do NOT change shared-helper codes — that would drift frame-capture).** The shared `ParseCamera`/`ParseFrameObjectTransforms`/`ValidateFrameObjects` throw `DirectorFrameValidationError` with generic codes (`invalid_input`, `unsupported_projection`, …). Replay **wraps** them at its own boundary and remaps:
  ...
  - UI-phase has **three** distinct doc-dependent failure modes (do not collapse them):
    - the `DirectorViewportGuard` ctor calls `ValidateSlice1ModelRhinoView`, which throws `DirectorFrameValidationError("unsupported_view", …)` when the active view is a page/non-model view → **surface `unsupported_view` as-is**;
    - **missing object** — replay checks existence itself first (`pDoc->LookupObject(uuid)` null/deleted) → `object_not_found {object_id}`;
    - **source-state drift** — `ValidateFrameObjects(pDoc, perFrameObjects[0])` (existence already passed, so it now fails only on the bbox check at `DirectorHandler.cpp:1308`) → catch `DirectorFrameValidationError` → `object_state_mismatch {object_id}` (the object exists but is not in its baked source pose).
  - Frame-capture keeps catching the same exceptions and emitting `invalid_input`/`unsupported_view` exactly as today — **the helpers are unchanged.**
...
- **Loop lambda (UI thread):**
  ...
  - Construct **one** `DirectorViewportGuard viewportGuard(pDoc);` (snapshots the pre-replay camera once; may throw `unsupported_view` — surface it) and **one** `DispatchDrainSuspension guard;` for the whole loop.
  - **UI-phase validation (doc-dependent):** for each object in `perFrameObjects[0]`, `pDoc->LookupObject(uuid)` → missing/deleted → `object_not_found {object_id}`. Then `ValidateFrameObjects(pDoc, perFrameObjects[0])` (only the source-bbox check can still fail) wrapped → `object_state_mismatch {object_id}`. (Object-set equality + `source_state` consistency across frames were proven worker-phase, so frame 0 covers all frames.)
