# LM5D Local Worker Turn Harness Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a deterministic one-turn local-worker harness that calls one worker callable exactly once and records a compact anchored activation outcome.

**Architecture:** LM5D is a new agent-layer module that composes the existing LM5A context, LM5B response, and LM5C disposition surfaces. It records worker exceptions and invalid worker return types as harness outcomes, while typed responses flow through LM5C. It does not add model calls, prompt rendering, dispatch, retries, graph mutation, stream continuation, or action execution.

**Tech Stack:** Python frozen dataclasses, `typing.Callable`, `typing.Literal`, pytest, AST import/call boundary checks.

---

## File Structure

- Create: `mcp_server/src/rook/agent/local_worker_turn_harness.py`
  - Owns the LM5D public harness seam.
  - Exports exactly `HarnessStatus`, `LocalWorkerTurnWorker`, `LocalWorkerTurnHarnessRecord`, and `run_local_worker_turn`.
  - Imports only LM5A/B/C surfaces plus stdlib dataclass/typing helpers.

- Create: `mcp_server/tests/test_local_worker_turn_harness.py`
  - Owns unit coverage, one offline integration proof, and AST/import boundary checks.
  - May import LM4 workflow compiler helpers for integration fixtures.
...
There are unrelated dirty RookChat files in the working tree. Do not stage or edit them as part of LM5D.
...
Create `mcp_server/tests/test_local_worker_turn_harness.py` with this content:
...
def test_completed_record_rejects_incoherence(
...
def test_unknown_action_response_is_completed_blocked_disposition() -> None:
    context = _minimal_context(action_ids=("draft_bind_params",))

    record = run_local_worker_turn(context, lambda received: _action_response("invented"))
...
def test_non_action_typed_responses_complete_with_matching_dispositions(
    response: LocalWorkerTurnResponse,
    disposition: str,
    reason: str,
) -> None:
    ...
    record = run_local_worker_turn(context, lambda received: response)
...
def test_non_response_worker_return_is_invalid_response_record(
    worker_return: object,
    type_name: str,
) -> None:
    context = _minimal_context()

    record = run_local_worker_turn(context, lambda received: worker_return)
...
def test_worker_exception_is_recorded_without_message_or_traceback() -> None:
    ...
    def worker(received_context: LocalWorkerTurnContext) -> LocalWorkerTurnResponse:
        raise ValueError("do not persist this message")
...
def test_base_exception_subclasses_propagate(
...
def test_lm5c_disposition_object_is_preserved(
    ...
    context = _minimal_context()
    response = _action_response()
    fake_disposition = _disposition_for(context, response)
...
```text
ModuleNotFoundError: No module named 'rook.agent.local_worker_turn_harness'
```

If the failure is anything other than the missing production module, fix the test
file before continuing.

- [ ] **Step 3: Commit the failing tests**

```powershell
git add mcp_server/tests/test_local_worker_turn_harness.py
git commit -m "test(lm5d): add local worker turn harness tests"
```

Expected result: commit succeeds and stages only the LM5D test file.

---

## Task 2: Add Production Harness Module
...
Create `mcp_server/src/rook/agent/local_worker_turn_harness.py` with this
content:
...
```powershell
git add mcp_server/src/rook/agent/local_worker_turn_harness.py mcp_server/tests/test_local_worker_turn_harness.py
git commit -m "feat(lm5d): add local worker turn harness"
```

Expected result: commit succeeds and stages only the LM5D production/test files.
...
- Verify: `mcp_server/src/rook/agent/local_worker_turn_harness.py`
- Verify: `mcp_server/tests/test_local_worker_turn_harness.py`

- [ ] **Step 1: Run targeted LM5D tests**
...
- [ ] **Step 3: Check LM5A/B/C source stayed untouched**
...
```powershell
rg -n "WorkerAllowedAction|compile_workflow_contract|snapshot_workflow_contract|load_workflow_contract_payload|run_current_step_stream|run_current_mapped_step|execute_mapped_step|map_accepted_proposal_to_step|revalidate_proposal|propose_next_node|CurrentStepRecord|EnvelopeSupplyRecord|EnvelopeSupplyResult|PlanGraph|CompiledWorkflowScaffold|CatalogCurrentStepProvider|WorkflowProvenanceEnvelopeSource|RookAgent|base_agent|dispatcher|server|litellm|OpenAI|Path|open\(|import json|json\.loads|json\.dumps|yaml|MappingProxyType|from collections\.abc import Mapping|import math|asyncio|threading|datetime|sleep|_freeze_json" mcp_server/src/rook/agent/local_worker_turn_harness.py
```
...
Expected output may still show the pre-existing unrelated RookChat dirty files:

```text
 M mcp_server/src/rook/agent/chat/chat_runner.py
 M mcp_server/tests/test_rookchat_tool_schema_golden.py
```

No LM5D files should be unstaged or uncommitted.

---

## Self-Review Checklist

- [ ] Public surface exports only `HarnessStatus`, `LocalWorkerTurnWorker`, `LocalWorkerTurnHarnessRecord`, and `run_local_worker_turn`.
- [ ] `run_local_worker_turn(...)` validates context and worker before invocation.
- [ ] Context anchors are read before invoking the worker.
- [ ] Worker callable is called exactly once with the exact context object.
...
- [ ] Completed records allow LM5C `blocked` dispositions.
- [ ] Harness record has no response-kind/action-id convenience fields.
- [ ] Harness record has no context object, worker id, model id, run id, timing, retry, fallback, critic, oversight, routing, authorization, dispatch, execution, stream, or graph-mutation fields.
- [ ] No production canned workers were added.
- [ ] No LM5A, LM5B, or LM5C production source files changed.
