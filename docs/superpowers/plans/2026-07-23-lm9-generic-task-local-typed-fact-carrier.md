...
### Task 1: Seal the pure JSON Schema profile and semantic-value registry
...
def test_expansion_counts_only_structural_child_schemas() -> None:
    ...
    assert admitted.expansion_units == 3
```
...
Expected: FAIL because the helper and code-owned contracts do not exist.
...
### Task 2: Validate complete typed values and prove unit-context authority
...
class VerifiedUnitContextIndex:
    entries: Mapping[tuple[str, str], VerifiedUnitContextEntry]
...
Include forward string/integer/Boolean/scalar, recipe-assumption four-field string/scalar, recipe-derived two-field string/integer/Boolean, and four-field scalar rows. Assert canonical bytes/fingerprint equal the existing Rook canonical JSON result and validation never changes the supplied mapping.
...
if required_presence in {"forward_fact", "recipe_assumption"}:
    ...
    if value["schema"] == "rook.semantic_scalar:v1":
        _require_exact_fields(value, FOUR_FIELDS)
    else:
        _require_fields_and_no_extras(
            value,
...
Require exact artifact shape/fingerprint, registered environment payload schema/fingerprint, payload validation, frozen sessions, trusted issuer, and `observed_at <= evaluated_at < expires_at`. Resolve each binding pointer and recompute the referenced value fingerprint. Issue `VerifiedUnitContextIndex` only inside the helper with a private proof token.

Scalar validation requires exactly one matching `rook.semantic_unit_context:v1` proof entry and never converts `model_unit` to `millimeter`.
...
### Task 3: Compose forward task envelopes, exact migration, and two unrelated witnesses
...
def reconstruct_historical_typed_values(
    parent_task_envelope: Mapping[str, object],
    *,
    historical_payload_schema: Mapping[str, object],
...
Expected: FAIL because the carrier composition module and fixtures do not exist.
...
Historical scalar reconstruction requires exact unit/context material already established by historical profile inputs; it may not infer from key names. If reconstruction is not exact, fail. Validate both sides independently, compare canonical bytes, then compare authority/provenance separately.
...
- [ ] **Step 8: Implement the code-owned negative-case manifest**

Expose stable IDs and executable builders for every row in Sections 17.1–17.5 of the approved design. Group them in five parameterized tables—registry/profile, typed value, fact/binding, unit context, and migration/genericity—but produce one exact result per ID. Every case must demonstrate a deterministic refusal after caller-authored outer identities are reclosed. The qualification consumes this same manifest; it does not infer coverage from pytest names.
...
### Task 4: Prove registry migration is outcome-neutral on exact evidence
...
### Task 5: Seal and independently verify the no-contact qualification witness
...
- [ ] **Step 1: Add the failing closed vertical witness**

Use a temporary destination. Invoke the real qualification entry point, then public verification. Assert exact schema/membership; exact commit/clean checkout; exact runtime/`jsonschema` identity; helper/profile/registry/payload/source/derivative identities; blocked parent; passing radial and annotation witnesses; one result per required negative case; and false model/provider/readiness/evaluator/compiler/Rhino/Grasshopper/product-authority flags.
...
Expected: FAIL because qualification writer/verifier/CLI do not exist.
