...
**Why this flow is safe (document this in the test docstring):** SUPERSEDED BY THE GATE RUN (2026-07-06, commit `1e3209fe`). The original "fixture evaporates via doc switch" flow described here was refuted by the first live run: `prepare_take`'s `document_not_saved` guard correctly refuses to switch away from a dirty document, which is exactly what the evaporation trick required. The as-run flow (normative; the committed test file is the authority over the draft code below): verify the live doc is UNMODIFIED → build the fixture → **save the (scratch) document** → package → prepare (doc is clean, guard passes) → verify artifacts → reopen the original document → **delete the fixture block definitions (with instances) and save again**, asserting object count and modified state return to pre-gate values. Run this gate on a SCRATCH document only — a mid-gate failure leaves the fixture saved in the document until cleanup is re-run.
