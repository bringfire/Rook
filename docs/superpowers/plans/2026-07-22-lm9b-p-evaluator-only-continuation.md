...
The verifier must derive the allowed optional evaluator files from `evaluator/attempt/capture.json`, require every checksummed path and role, reject every unchecksummed/extra file, and reject all `compiler`, `handoff`, `checkpoint-2`, and successor-policy members. It must bind archived source bytes to the production pins; pass those bytes plus the exact code-owned corrected rubric through the shared frozen-input constructor, gate, renderer, fixed system prompt, and provider-request builder; and require the resulting protocol identity, allowed delta, gate, rendered request, provider request, and instrument identity to match the archive exactly. It then reconstructs the evaluator result from normalized assistant evidence and exact tool arguments through the shared pure parser, reruns readiness from archived inputs and verification time, validates invocation and dispatch equations, and recomputes classification from that derived result plus staged recipe bytes. The private staging/finalization verifier consumes the already verified in-memory rubric snapshot and permits the identity-bound staging path only before rename; the public verifier loads the code-owned rubric and requires the runtime archive path to equal the canonical destination.
