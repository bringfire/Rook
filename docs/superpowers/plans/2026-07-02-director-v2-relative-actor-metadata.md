...
**Goal:** Implement strict v2 Director actor metadata generation, source-occurrence capture, resolution, runtime reading, and explicit v1-to-v2 migration using `.rook/...` project-relative refs rooted at the active `.3dm` directory.
...
- Modify `mcp_server/tests/test_multi_instance_targeting.py`
  - Explicitly assert targeting policies for
    `rhino_director_capture_source_occurrence_v2`,
    `rhino_director_write_actor_metadata_v2`, and
    `rhino_director_read_actor_metadata_v2`.
...
No native C++, managed C#, or `.vcxproj` file is modified in this plan unless
the current native block routes prove unable to expose recursive occurrence
inventory. The V2 `.gh` file is updated only as a live validation artifact so
Lane A is executable as an agent-mediated capture flow through the production
source-occurrence capture and writer path.

### Task 0: Convert Prototype Source-Occurrence Capture Scope

**Files:**
- Modify: `docs/superpowers/specs/2026-07-02-director-v2-relative-actor-metadata-design.md`
- Modify: `docs/superpowers/plans/2026-07-02-director-v2-relative-actor-metadata.md`

This task corrects the original plan boundary. The pathing conversion is not
complete if the source-occurrence capture previously represented by prototype
take manifests remains outside the v2 toolchain.

- [ ] **Step 1: State the missing production tool**

Add `rhino_director_capture_source_occurrence_v2` to the spec and this plan as
part of Lane A.

- [ ] **Step 2: Demote prototype take archives**

State that `.rook/director_takes/**` is reference-only prototype material. It
must not be required by runtime readers, writers, or Lane A validation.

- [ ] **Step 3: Update acceptance**

Lane A acceptance requires:

- source-occurrence capture generated by the v2 tool;
- actor/grouping metadata generated by `rhino_director_write_actor_metadata_v2`;
- no hand-authored source-occurrence JSON;
- no runtime dependency on `director_takes`.

---

### Task 2A: v2 Source-Occurrence Capture Tool
...
**Production capture entry point:** `rook.director_actor_metadata.capture_source_occurrence_v2(...)`.
The MCP tool `rhino_director_capture_source_occurrence_v2` wraps this function.

Required behavior:

- resolve the active saved `.3dm` through `/document`;
- use explicit `ids` when supplied, otherwise `/selection`;
- reject empty selection;
- capture top-level selected document object facts;
- capture block instance and block definition facts for `InstanceReference`
  selections through existing `/block/info`, `/block/instances`, and
  `/block/objects-detailed` routes;
- include direct definition inventory and nested instance counts available from
  those routes;
- write a v2 `director_selection_snapshot`;
- return a `.rook/...` ref and runtime `resolved_snapshot_path`;
- never write `.rook/director_takes`.

The default snapshot id should be deterministic enough for repeated validation:

```text
source_occurrence_<safe label or first object id short hash>
```

Pearson Lane A should supply:
