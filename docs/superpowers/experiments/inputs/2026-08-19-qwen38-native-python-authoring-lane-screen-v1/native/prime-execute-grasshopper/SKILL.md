...
- `gh_snapshot` for the initial canvas and the final receipt-fenced checkpoint;
- `gh_library` for ranked component identity discovery;
- `gh_batch_component_info` for exact port metadata;
- `gh_edit` for the smallest coherent native scaffold and local corrections;
- `gh_errors` for diagnostics;
- `gh_wait_for_solve_readiness` for receipt readiness before the final snapshot.
...
capabilities only. If one cannot supply a required fact, report that blocker
instead of widening the route.

Read each listed contract directly once before its first call. Do not search for
a listed capability name. Resolve only the component identities and parameter
indices required by the smallest next scaffold, then mutate. Further discovery
must name the exact unresolved identity or port fact needed by the next intended
mutation and stop once that fact is resolved.

Treat a failed component or connection as local first. Repair only the affected
execution-owned subgraph from current live evidence. Preserve unaffected
committed work.
<!-- authoring-lane:end -->
