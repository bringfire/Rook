...
> - **NEXT: Task 6** — `OcctAdjacencyEngine` kernel (`Common(faceA,faceB).Area` over converted faces, bbox prefilter, `facePairs`, serialized) + OCCT-only offline robustness matrix + in-plugin `Evaluate` validation. Then Task 7 (diagnostics), Task 8 (single-commit Phase-2 integration: strip old types, wire engine, retire Clipper+tracer+dev routes), Task 9 (pin OCCT 7.9.3, `$(OcctRoot)`, measured DLL closure, LGPL), Task 10 (live verify on the real `/scene/graph/adjacency/exact` route).
