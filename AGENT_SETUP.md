...
| **Git** | `git --version` | Only for source install |
...
1. Download the latest `Rook-Setup-<version>.exe` from:
   `https://github.com/bringfire/Rook/releases`
...
**macOS/Linux (MCP server only):**
```bash
chmod +x install.sh && ./install.sh
```

The script installer uses [uv](https://docs.astral.sh/uv/) and downloads Python
automatically if needed. Building Rhino plugins from source requires Visual Studio
2022 with the C++ Desktop workload, MFC, and .NET Framework 4.8. See
**[BUILDING.md](BUILDING.md)** for the exact build chain, toolset versions, and
step-by-step commands.

**Agent callers:** `install.ps1` supports flags for programmatic use:
- `-DryRun -Json` — prerequisite check with zero side effects, summary on stdout
- `-RequireNative` — fail hard (exit `1`) if C++ prerequisites missing
- `-SkipNative` / `-SkipChirp` / `-SkipConfig` / `-NoVerify` — skip individual steps

The script writes `install-summary.json` incrementally after each step and exits
with code `0` (functional), `1` (hard failure), or `2` (partial — runtime exists
but Rhino bridge not functional). See [BUILDING.md](BUILDING.md) for the full
flag and summary reference.
...
- `<path-to-python>`: `<repo>/mcp_server/.venv/Scripts/python.exe`
- `<path-to-mcp_server-directory>`: `<repo>/mcp_server`
- `<install-root>`: `<repo>`
- `<data-directory>`: `<repo>/knowledge`
- `ROOK_MODE`: `dev`
...
### Repo-Scoped Config (`.mcp.json`)

For source installs, `.mcp.json` in the repo root configures the MCP server for
Claude Code when working inside the repo directory. This is auto-generated by
`install.ps1`.
