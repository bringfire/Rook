...
@pytest.mark.asyncio
async def test_gh_execute_intent_is_absent_from_registered_tools():
    """The retired identity is no longer advertised on the public surface."""
    tools = await server.list_tools()
    assert all(tool.name != "gh_execute_intent" for tool in tools)
