...
def _build_gh_python_preamble(pins_in: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "def _gh_cast_warn(_i, _cast, _v):",
        '    """Emit one GH runtime warning per cast-failure fallback. Defensive — never raises, never breaks the solve. See issue #50."""',
        "    try:",
        "        import Grasshopper as _gh",
        "        ghenv.Component.AddRuntimeMessage(",
        "            _gh.Kernel.GH_RuntimeMessageLevel.Warning,",
        '            "Pin %d: cast to %s failed for value of type %s; substituted %s(0)" % (_i, _cast.__name__, type(_v).__name__, _cast.__name__)',
        "        )",
        ...
        "    # Scan every wrapper across every branch; return the first non-None Value.",
        "    # Matches GH's native item-access semantics which skip null wrappers rather",
        "    # than reporting no-data on the first null.",
        "    _warned = False",
        ...
        "                try: _v = _cast(_v)",
        "                except Exception:",
        "                    if not _warned: _gh_cast_warn(_i, _cast, _v); _warned = True",
        "                    _v = _cast(0)  # bad-cast falls back to numeric default",
        "            return _v",
        "    return None",
        "",
...
- Mirror: {"ids": ["guid"], "operation": "mirror", "planeOrigin": [0, 0, 0], "planeNormal": [1, 0, 0]}""",
        ...
        Tool(
            ...
            description=(
                ...
                "error record; batch continues. Error codes emitted in errors[].error:\n"
                "  - shape errors: invalid_name, invalid_set, invalid_rename, invalid_parent, "
                "invalid_color, invalid_plot_color, invalid_plot_weight, invalid_linetype, "
                "invalid_linetype_index, invalid_material, invalid_material_index, invalid_visible, "
                "invalid_locked\n"
                "  - resolution errors: not_found, parent_not_found, linetype_not_found, "
                "material_not_found, name_collision, cycle_detected\n"
                "  - other: no_changes, modify_failed, exception\n\n"
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "allow_experimental_model": {"type": "boolean", "description": "Dev/test override: allow submitting an experimental catalog model. Honored only with an explicit model_id; never affects default model resolution. Omit for normal use."},
                    "port": {"type": "integer", "description": "Specific Rhino port to target."},
                },
