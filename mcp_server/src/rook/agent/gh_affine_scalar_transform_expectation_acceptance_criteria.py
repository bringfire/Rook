...
def affine_scalar_transform_action_selection_contract() -> dict[str, Any]:
    return {
        "contract_id": "gh_affine_scalar_transform_action_selection:v1",
        "worker_agency": (
...
def assemble_gh_affine_scalar_transform_expectation_packet(
    sources: GhAffineScalarTransformExpectationSources,
) -> dict[str, Any]:
    _validate_sources(sources)
    source_classes = {
        sources.expected_output_contract.source_class,
        sources.factor_contract.source_class,
    ...
    source_paths = {
        ...
        sources.factor_contract.source_path,
    ...
    packet = {
        "schema": GH_AFFINE_SCALAR_TRANSFORM_EXPECTATION_PACKET_SCHEMA,
        "source_set": {
...
def project_gh_affine_scalar_transform_expectation_legacy(
    packet: dict[str, Any],
) -> dict[str, Any]:
    fields = packet.get("fields")
    if not isinstance(fields, dict):
        raise ValueError("GH affine scalar transform packet fields missing")
    acceptance_criteria = fields.get("acceptance_criteria")
    if not isinstance(acceptance_criteria, dict):
        raise ValueError("GH affine scalar transform acceptance_criteria missing")
    return {
...
def _validate_sources(sources: GhAffineScalarTransformExpectationSources) -> None:
    _validate_source(
    ...
    _finite_number(sources.factor_contract.value, "factor_contract")
...
def _validate_source(
    source: GhAffineScalarTransformExpectationSource,
    *,
    source_class: str,
    source_path: str,
) -> None:
...
def _validate_projection(value: Any) -> None:
    ...
    if value != EXPECTED_AFFINE_PROJECTION:
        raise ValueError(
            "projection_contract value must match EXPECTED_AFFINE_PROJECTION"
        )
...
def _validate_fixture_anchor(value: Any) -> None:
    ...
    if value["projection_id"] != EXPECTED_AFFINE_PROJECTION["projection_id"]:
        raise ValueError(
            "fixture_anchor projection_id must match EXPECTED_AFFINE_PROJECTION"
        )
