---
name: prime-execute-grasshopper-python
description: Explicitly selected Python scripting lane for Grasshopper authoring on the currently pinned live document.
---
...
## Experimental Python authoring lane

This row is routed to the Python authoring lane. Implement the requested core
geometry in one Python script component with ordinary Grasshopper controls. The
script route is immediately available; do not evaluate or build a native
component alternative.

These capabilities are known and immediately available:

- `gh_snapshot` for the initial canvas, solved outputs, and final receipt-fenced
  checkpoint;
- `gh_create_script` for single-shot script creation with
  `language="python"`, source, input pins, and output pins;
- `gh_set_script_pins` for preparatory signature changes only;
- `gh_update_script` for source updates and the terminal receipt after any pin
  change;
- `gh_edit` for ordinary controls, connections, values, grouping, and local
  graph corrections;
- `gh_errors` for diagnostics;
- `gh_wait_for_solve_readiness` for receipt readiness before fenced evidence.

For this experimental row, this lane section overrides later generic advice to
search for a hidden capability or poll `gh_status`. Use the listed Rook
capabilities, except for the narrowly permitted native discovery below. If they
cannot supply a required fact, report that blocker instead of widening the
route.

Read each listed contract directly once before its first call. Do not search for
a listed capability name. Create the smallest working script scaffold as soon
as the creation and edit contracts are known.

For script creation, use rich pin objects when access, optionality, or geometry
type matters. Assign actual RhinoCommon geometry to geometry outputs. Connect
ordinary controls to the script with `gh_edit`. If the signature must change,
call `gh_set_script_pins`, then `gh_update_script`; the pin call is preparatory
and does not establish terminal solve readiness by itself.

Treat compile, runtime, pin, or connection failure as local first. Inspect the
affected live evidence and update only the script or owned connection that is
wrong. Native discovery is allowed only for an ordinary control that `gh_edit`
cannot express or for a named blocker whose resolution is required by the next
mutation. Do not switch implementation modes.
<!-- authoring-lane:end -->
