...
function Test-WheelhouseBuilderEnforcesReleaseContracts {
    ...
    Assert-Contains -Text $code -Expected 'Stage-LockedDependencies -PythonExe $buildPythonExe -RequirementsLock $admittedDependencyLock' -Message 'Wheelhouse builder must acquire dependency wheels using the admitted input lock.'
    Assert-Contains -Text $code -Expected 'Invoke-CheckedProcess -FilePath $PythonExe -Arguments $arguments' -Message 'Locked acquisition must retain timeout-bounded process ownership.'
    Assert-Contains -Text $code -Expected '''--no-deps'' , ''--require-hashes''' -Message 'Dependency acquisition must not expand beyond hashed approved inputs.'
