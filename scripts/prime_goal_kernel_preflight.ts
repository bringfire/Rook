...
async function main(): Promise<void> {
	const [primeRoot, evidenceRoot, rawBudget] = process.argv.slice(2);
	const budget = Number(rawBudget);
	if (!primeRoot || !evidenceRoot || !Number.isSafeInteger(budget) || budget <= 0) {
		throw new Error("usage: prime_goal_kernel_preflight <prime-root> <evidence-root> <token-budget>");
	}
