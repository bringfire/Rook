...
The public docs and showcase site for [Rook](https://github.com/bringfire/Rook),
...
```sh
npm install
npm run dev      # http://localhost:4321/Rook/
```
...
1. Push this folder as the root of the public repo `bringfire/Rook`.
2. In the repo settings, set **Pages → Build and deployment → Source** to
   **GitHub Actions**.
3. The site publishes to `https://bringfire.github.io/Rook/`.
...
2. Update the `/Rook/...` links in `src/content/docs/index.mdx` (hero actions) to
   root-relative paths.
