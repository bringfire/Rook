...
Rook is proprietary software (see the [License](/Rook/legal/eula/)). The
implementation — the MCP server, the native and companion plug-ins, the chat
server, the knowledge graph, and the agent runtime — isn't open for
redistribution, and you don't need it to use Rook.

Found a bug, have a question, or want to request a feature? Open an issue on the
[Rook repository](https://github.com/bringfire/Rook/issues) or email
